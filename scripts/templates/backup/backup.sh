  ...
  if [ -n "$_DB_LIST" ]; then
    ...
    for _database in $_DB_LIST; do
      ...
      for _DB_NAME in $_DB_NAMES; do
        ...
        case $_DB_TYPE in
          mysql)
            ssh -C -E $_LOG_FILE -p $_HOST_PORT "$_HOST_USER@$_HOST_ADDRESS" "MYSQL_PWD=$_DB_PASS mysqldump -u $_DB_USER $_DB_NAME | gzip -9 > /tmp/$_BACKUP_FILE"
            ;;

          postgresql)
            ssh -C -E $_LOG_FILE -p $_HOST_PORT "$_HOST_USER@$_HOST_ADDRESS" "PGPASSWORD=$_DB_PASS pg_dump -U $_DB_USER -d $_DB_NAME | gzip -9 > /tmp/$_BACKUP_FILE"
            ;;
        esac
        ...
        if [ -e "$_BACKUP_FILE" ]; then
          ...
          _NUMBER_FILES=$(ls $_DB_NAME*.gz 2> /dev/null | wc -l)

          if [ "$_NUMBER_FILES" -gt "$_MAX_FILES" ]; then
            let _NUMBER_REMOVALS=$_NUMBER_FILES-$_MAX_FILES

            _FILES_REMOVE=$(ls $_DB_NAME*.gz | head -n "$_NUMBER_REMOVALS")
